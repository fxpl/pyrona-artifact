...
_PYTHON_BIN_SUFFIX_DEFAULT=""
case "$(uname -s)" in
  CYGWIN*|MINGW*|MSYS*|Darwin)
    _PYTHON_BIN_SUFFIX_DEFAULT=".exe"
    ;;
esac
PYTHON_BIN_SUFFIX="${PYTHON_BIN_SUFFIX:-${_PYTHON_BIN_SUFFIX_DEFAULT}}"
BASELINE_PYTHON_BIN="${BASELINE_PYTHON_BIN:-${BASELINE_PYTHON_DIR}/python${PYTHON_BIN_SUFFIX}}"
PATCHED_PYTHON_BIN="${PATCHED_PYTHON_BIN:-${PATCHED_PYTHON_DIR}/python${PYTHON_BIN_SUFFIX}}"

unset _SNAPSHOT_SOURCES_DIR
unset _PYTHON_BIN_SUFFIX_DEFAULT
EOF
} > "$SOURCES_PATH"
